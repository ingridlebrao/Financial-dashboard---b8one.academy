...
function renderCardWeeklyLimitProgressByIndex(currentCard, cardIndex) {
  const weeklyPaymentLimitProgressDiv = document.querySelector(
    ".credit-card-info__progress-bar"
  );
  const weeklyPaymentUsedPercentage =
    (currentCard.weeklyPayment.used / currentCard.weeklyPayment.limit) * 100;
...
function renderCardDataByIndex(cardIndex) {
  ...
  const currentBalanceSpan = document.querySelector(
    ".credit-card-info__text.credit-card-info__text--blue.credit-card-info__text--dollar"
  );
  currentBalanceSpan.innerHTML = currentCard.currentBalance;

  const incomeSpan = document.querySelector(
    ".credit-card-info__text.credit-card-info__text--green.credit-card-info__text--dollar"
  );
  incomeSpan.innerHTML = currentCard.income;

  const outcomeSpan = document.querySelector(
    ".credit-card-info__text.credit-card-info__text--red.credit-card-info__text--dollar"
  );
  ...
  const weeklyPaymentLimitSpan = document.querySelector(
    ".credit-card-info__text.credit-card-info__text--black"
  );
